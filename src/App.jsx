...
const socket = io('https://gemhunterserver-production.up.railway.app', {
...
function App() {
  ...
  useEffect(() => {
    ...
    socket.on('huntersHiredNotification', (notification) => {
      ...
      toast.info(
        ...
        } at ${moment(notification.time).format('h:mm:ss a')}`
      );
    ...
    socket.on('protectionPurchasedNotification', (notification) => {
      ...
      displayToast(
        notification.message,
        notification.protectionEndTime,
        notification.time
      );
      setIsNewNotification(true);
    });
  ...
  const NotificationsData = async () => {
    try {
      const res = await axios.get(
        `${ApiUrl}notification/${address ? address : ''}`
      );
  ...
  const displayToast = (message, detail, time) => {
    ...
    toast.info(
      `${message} ${detail ? `| Detail: ${detail}` : ''} at ${moment(
        time
      ).format('h:mm:ss a')}`
    );
  };
    ...
    <>
      <BrowserRouter>
        <Suspense fallback={<Loader />}>
          <Header
            isNewNotificationOne={isNewNotification}
            notifications={notifications}
            setIsNewOneNotification={setIsNewNotification}
          />
          <Routes>
